  ...
  const handleInvoiceUpload = async (event, rqid) => {
    ...
    try {
      ...
      if (document.getElementById("invoice_" + rqid)) {
        formData.append(
          "invoice",
          document.getElementById("invoice_" + rqid).files[0],
          `invoice_${rqid}.pdf`
        );
      }
  ...
  const handlePoUpload = async (event, rqid) => {
    ...
    try {
      ...
      if (document.getElementById("po_" + rqid)) {
        formData.append(
          "po",
          document.getElementById("po_" + rqid).files[0],
          `po_${rqid}.pdf`
        );
      }

      const response = await axios.post(
        "http://localhost:5000/admin/PO",
        formData
      );
      console.log(response);
      alert("Request updated!!");
        ...
        {selectedItem && (
          <div className="po-card-view">
            <div className="po-card-header">
              ...
              <p>
                {selectedItem.spec} - {selectedItem.quantity}
              </p>
            </div>
            <div className="po-quote-info">
              <div className="po-quote-amount">
                Quote Amount: <b>{selectedItem.vendor[0].quote_amount}</b>
              </div>
              <a
                className="po-download-button"
                onClick={(e) =>
                  handleQuoteDownload(e, selectedItem.vendor[0].rqid)
                }
              >
                Download Quote
              </a>
            ...
            <div className="po-upload-buttons">
              {selectedItem.vendor[0].po ? (
                <a
                  className="po-download-button"
                  onClick={(e) =>
                    handlePoDownload(e, selectedItem.vendor[0].rqid)
                  }
                  target="_blank"
                ...
                <>
                  ...
                  <DragDropFile
                    name={`po_${selectedItem.vendor[0].rqid}`}
                    id={`po_${selectedItem.vendor[0].rqid}`}
                    selectedFile={
                      selectedOptions[selectedItem.vendor[0].rqid]?.invoice
                    }
                    setSelectedFile={(file) =>
                      handlePoUpload(file, selectedItem.vendor[0].rqid)
                    }
                  />
                </>
              )}
              ...
              {selectedItem.vendor[0].invoice ? (
                <button
                  ...
                  onClick={(e) =>
                    handleInvoiceDownload(e, selectedItem.vendor[0].rqid)
                  }
                  target="_blank"
                  rel="noopener noreferrer"
                >
                ...
                <>
                  ...
                  <DragDropFile
                    name={`invoice_${selectedItem.vendor[0].rqid}`}
                    id={`invoice_${selectedItem.vendor[0].rqid}`}
                    selectedFile={
                      selectedOptions[selectedItem.vendor[0].rqid]?.invoice
                    }
                    setSelectedFile={(file) =>
                      handleInvoiceUpload(file, selectedItem.vendor[0].rqid)
                    }
                  />
                </>
              )}
