...
export default function Vp({ initialOpenId }) {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/vp`);
  ...
  const handleFormSubmit12 = async (status, rqid) => {
    try {
      ...
      const response = await axios.post(`http://localhost:5000/vp`, formData);
      ...
      <div className="vendor-list2">
        <Accordion className="acc-item">
          {sortedData.map((request) => (
            <AccordionItem
              isActive={request.id == initialOpenId}
            ...
            <div styles={{ padding: "0px auto" }}>
              ...
              <div className="quote-info">
                <div className="quote-amount">
                  Quote Amount: <b>{vendor.quote_amount}</b>
                </div>
                <a
                  className="download-button"
                  onClick={(e) => handleQuoteDownload(e, vendor.rqid)}
                >
                  Download Quote
                </a>
              </div>
              <textarea
                id={"t_area_" + vendor.rqid}
              ...
              <div className="action-buttons">
                <button
                  onClick={() => {
                    handleFormSubmit12(1, vendor.rqid);
                  }}
                ...
                <button
                  onClick={() => {
                    handleFormSubmit12(2, vendor.rqid);
                  }}
